...
ElementsTable.Dropdown = (function()
	...
	function Element:New(Idx, Config)
		...
		local Dropdown = {
			...
			IsToggling = false,
			IsBuilding = false -- เพิ่ม flag เพื่อป้องกันการ build ซ้ำ
		}
		...
		-- ฟังก์ชันสร้าง button แบบ lazy loading
		local function CreateButton(Value, Idx)
			local Table = {}
		...
		function Dropdown:BuildDropdownList()
			if Dropdown.IsBuilding then
				return
			end
			
			Dropdown.IsBuilding = true
			local Values = Dropdown.Values
			local Buttons = {}

			-- ลบ button เก่าทั้งหมด
			for _, Element in next, DropdownScrollFrame:GetChildren() do
				if not Element:IsA("UIListLayout") then
					Element:Destroy()
				end
			...
			-- ใช้ coroutine แทน task.spawn เพื่อควบคุมการทำงานได้ดีกว่า
			coroutine.wrap(function()
				local batchSize = 50 -- จำนวนที่จะสร้างแต่ละ batch
				local totalCount = #Values
				
				for i = 1, totalCount, batchSize do
					local endIndex = math.min(i + batchSize - 1, totalCount)
					
					-- สร้าง button แต่ละ batch
					for j = i, endIndex do
						local Value = Values[j]
						local Button, Table = CreateButton(Value, j)
						Buttons[Button] = Table
						
						-- คำนวณขนาด text ที่ใหญ่ที่สุด
						if Button.ButtonLabel and Button.ButtonLabel.TextBounds.X > ListSizeX then
							ListSizeX = Button.ButtonLabel.TextBounds.X
						end
					end
					
					-- รอ 1 frame ก่อนสร้าง batch ถัดไป
					RunService.Heartbeat:Wait()
				end
				
				-- เก็บ reference ของ buttons
		...
		if next(Defaults) then
			...
			-- รอให้ BuildDropdownList เสร็จก่อนจึงแสดงผล
			coroutine.wrap(function()
				while Dropdown.IsBuilding do
					task.wait()
				end
				Dropdown:Display()
			end)()
