...
Multi=p.Multi,
Callback=p.Callback or function()end,
SearchEnabled=p.SearchEnabled ~= false, -- เปิดการค้นหาโดยค่าเริ่มต้น
SearchPlaceholder=p.SearchPlaceholder or "Search...",

UIElements={},

Opened=false,
Tabs={},
FilteredValues={}, -- เก็บค่าที่ผ่านการกรอง
SearchText="" -- ข้อความค้นหา
}
...
-- Search container จะถูกเพิ่มที่นี่
...
-- ฟังก์ชันค้นหา
function q.FilterValues(s, searchText)
q.SearchText = searchText or ""
if q.SearchText == "" then
q.FilteredValues = q.Values
else
q.FilteredValues = {}
local lowerSearch = string.lower(q.SearchText)
for _, value in ipairs(q.Values) do
if string.find(string.lower(value), lowerSearch, 1, true) then
...
local function RecalculateCanvasSize()
local scrollFrame = q.UIElements.Menu.Frame.ContentFrame.ItemsScrollFrame
scrollFrame.CanvasSize=UDim2.fromOffset(0,q.UIElements.UIListLayout.AbsoluteContentSize.Y)
end
...
function q.Refresh(s,t)
local scrollFrame = q.UIElements.Menu.Frame.ContentFrame.ItemsScrollFrame
for u,v in next,scrollFrame:GetChildren()do
if not v:IsA"UIListLayout"then
v:Destroy()
end
end
...
local baseHeight = q.SearchEnabled and 38 or 0 -- เพิ่มความสูงสำหรับ search box
local itemCount = #q.FilteredValues
local maxHeight = 392 + baseHeight
local calculatedHeight = q.UIElements.UIListLayout.AbsoluteContentSize.Y + (n.MenuPadding*2) + baseHeight


if itemCount > 10 then
q.UIElements.MenuCanvas.Size=UDim2.fromOffset(q.UIElements.MenuCanvas.AbsoluteSize.X, maxHeight)
else
q.UIElements.MenuCanvas.Size=UDim2.fromOffset(q.UIElements.MenuCanvas.AbsoluteSize.X, calculatedHeight)
end
end
...
-- คำนวณพื้นที่ที่เหลือด้านล่าง
local u=g.ViewportSize.Y-(s.AbsolutePosition.Y+s.AbsoluteSize.Y)-n.MenuPadding-54
local v=t.AbsoluteSize.Y+n.MenuPadding

-- คำนวณพื้นที่ที่เหลือด้านบน
local availableTop = s.AbsolutePosition.Y - n.MenuPadding - 54
local maxHeightBelow = u - 20 -- เผื่อไว้นิดหน่อย
local maxHeightAbove = availableTop - 20

local w=-54
local showAbove = false

-- ถ้าพื้นที่ด้านล่างไม่เพียงพอ แต่ด้านบนมีพื้นที่มากกว่า
if u < v and availableTop > u then
showAbove = true
-- ปรับขนาด dropdown ให้พอดีกับพื้นที่ด้านบน
local maxAllowedHeight = math.min(v, maxHeightAbove)
q.UIElements.MenuCanvas.Size = UDim2.fromOffset(
q.UIElements.MenuCanvas.AbsoluteSize.X, 
maxAllowedHeight
)
else
-- แสดงด้านล่าง แต่ปรับขนาดให้ไม่เลยจอ
local maxAllowedHeight = math.min(v, maxHeightBelow)
if u < v then
w = v - u - 54
-- ปรับขนาด dropdown ให้พอดีกับพื้นที่ที่เหลือ
q.UIElements.MenuCanvas.Size = UDim2.fromOffset(
q.UIElements.MenuCanvas.AbsoluteSize.X, 
maxAllowedHeight
)
...
s.AbsolutePosition.X+s.AbsoluteSize.X,
0,
s.AbsolutePosition.Y - t.AbsoluteSize.Y - n.MenuPadding
)
else
...
-- ตรวจสอบไม่ให้เลยขอบซ้ายและขวาของจอ
local rightEdge = t.AbsolutePosition.X + t.AbsoluteSize.X
if rightEdge > g.ViewportSize.X then
local adjustment = rightEdge - g.ViewportSize.X + 20
t.Position = UDim2.new(0, t.AbsolutePosition.X - adjustment, 0, t.AbsolutePosition.Y)
end

if t.AbsolutePosition.X < 20 then
t.Position = UDim2.new(0, 20, 0, t.AbsolutePosition.Y)
end
end
...
-- เคลียร์การค้นหาเมื่อเปิด แต่ไม่โฟกัส textbox ทันที
if q.SearchEnabled then
local searchInput = q.UIElements.SearchBox.SearchInput
searchInput.Text = ""
q:FilterValues("")
-- ไม่โฟกัส textbox ทันที ให้ผู้ใช้กดเอง
end
